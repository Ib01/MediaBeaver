...
public class MovieRegExController 
{
	...
	*/
	...
	public @ResponseBody MovieRegEx testRegEx(@Valid @RequestBody MovieRegEx model, BindingResult result ) 
	{   
		RegExGenerator reg = new RegExGenerator();
	...
	private boolean addErrors(RegExGenerator reg, MovieRegEx model, BindingResult result)
	{
		boolean ret = false;
	    ...
	    {
	    	model.getErrors().add(new JsonError(error.getField(), error.getDefaultMessage()));
	    	ret = true;
	    }
		
		//throw error if assembled name does not contain a group number 
		if(!model.fieldHasError("nameParser.assembledItem") && !reg.containsCaptureGroup(model.getNameParser().getAssembledItem()))
		{
			model.getErrors().add(new JsonError("nameParser.assembledItem", "This field must contain at least 1 group number in the format {n} where n is a number"));
			ret = true;
		}
		
		//throw error if assembled name does not contain a group number 
		if(!model.fieldHasError("yearParser.assembledItem") && !reg.containsCaptureGroup(model.getYearParser().getAssembledItem()))
		{
			model.getErrors().add(new JsonError("yearParser.assembledItem", "This field must contain at least 1 group number in the format {n} where n is a number"));
			ret = true;
		}
		
		/*if(caps.size() == 0)
		{
			model.getErrors().add(new JsonError("expression", "This field must contain 2 or more capture groups to capture the movie name and year"));
			ret = true;
		}*/
	...
	//this method response to POST request http://localhost/spring-mvc-json/rest/cont/person
	  ...
	  public @ResponseBody MovieRegEx post(HttpServletResponse res) 
	  {    
